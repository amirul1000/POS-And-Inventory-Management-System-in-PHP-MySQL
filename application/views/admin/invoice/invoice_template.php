	...
	<tr>
		<td width="30%">
            ...
            if (is_file(APPPATH . '../public/' . $company[0]['file_company_logo']) && file_exists(APPPATH . '../public/' . $company[0]['file_company_logo'])) {
                ?>
			  <img
			src="<?php echo base_url().'public/'.$company[0]['file_company_logo']?>"
			class="picture_100x100">
			  <?php
            } else {
                ?>
            <img
...
<table cellspacing="3" cellpadding="3" class="table" align="left">
	...
	<tr>
		<td><?=html_entity_decode($customers['address'])?>
           <br>
          <?=html_entity_decode($customers['zip'])?>,<?=html_entity_decode($customers['city'])?>,<?=html_entity_decode($customers['state'])?></td>
	</tr>
	<tr>
		<td><?=html_entity_decode($customers['phone_no'])?></td>
	</tr>
	<tr>
		<td><?=html_entity_decode($customers['email'])?></td>
	</tr>
</table>
...
<table cellspacing="3" cellpadding="3" class="table" align="center">
	<tr>
		...
		<td><?=html_entity_decode($invoice['date_of_invoice'])?></td>
	</tr>
	<tr>
		<td>Description</td>
		<td><?=html_entity_decode($invoice['description'])?></td>
	</tr>
	<tr>
		<td>Total cost</td>
		<td><?=html_entity_decode($invoice['total_cost'])?></td>
	</tr>
	<tr>
		<td>Amount paid</td>
